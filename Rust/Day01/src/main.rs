fn main() {
    let input_string: String = String::from("heightseven4two5
    npskfdstpk2knsm
    ...
    let x = get_total_value(parts);

    println!("{}", x);
}

fn get_value_from_string(string_value: &str) -> u32 {
    let trimmed_string = string_value.trim();

    let bytes_string = trimmed_string.chars();
    let mut value_vec: Vec<String> = Vec::new();

    for s in bytes_string {
        if s.is_numeric() {
            value_vec.push(s.to_string());
        }
    }

    let first_number = value_vec.first().expect("err");
    let last_number = value_vec.last().expect("err");

    let format_number = format!("{}{}", first_number, last_number);

    format_number.parse::<u32>().expect("kek")
}

fn get_total_value(string_list: Vec<&str>) -> u32 {
    let mut values: Vec<u32> = Vec::new();
    let mut total = 0;

    for line in string_list {
        values.push(get_value_from_string(line));
    }
