const input: string = `heightseven4two5
npskfdstpk2knsm
...
console.log(sumUpValues(input));


function getEncryptedValue(val: string): number {
    const chars: string[]  = val.split("");
    let numbersArray: string[] = [];
    for (const s of chars) {
        if (!isNaN(parseInt(s))) numbersArray.push(s);
    }
...
function sumUpValues(input: string) {
    const stringLines = input.split("\n");
    const values = [0];

    for (const line of stringLines) {
        values.push(getEncryptedValue(line));
    }
