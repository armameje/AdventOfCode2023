...
namespace Day02_Part02
{
    internal class Program
    {
        ...
        static int GetMaxPower(string values)
        {
            var maxPowerPerGames = new List<int>();
            int total = 0;
            ...
            using (StringReader reader = new StringReader(values))
            {
                ...
                {
                    var game = ParseGameMaxValue(line);

                    maxPowerPerGames.Add(game.RedCube * game.GreenCube * game.BlueCube);
                }

            }
            foreach (var item in maxPowerPerGames)
            {
                total += item;
            }
